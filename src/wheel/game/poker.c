...
bool poker_send_str(Poker *poker, const char *str) {
	isize bytes_sent = send_str_to(poker->socket, poker->server, str, 0);
	++poker->packets_sent;

	// NOTE: this is almost impossible to happen because UDP is connectionless
	if (bytes_sent <= 0) {
		lol_error_e("failed to send: %s", str);
		return false;
	}

	poker->t0 = time_now();
	lol_info("sent request to server: %s", str);
...
void *login(void *arg) {
	...
	// get version
	if (!poker_send_str(poker, "version")) {
		return NULL;
	}
	...
	// login
	if (poker->id > 0) {
		if (!poker_send_str(poker, TextFormat("id %d", poker->id))) {
			return NULL;
		}
