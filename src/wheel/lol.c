...
static lol_inline FILE *get_default_std_target() { return stderr; }
...
static lol_inline void lol_compose_str(
	...
	if (lol_likely(!content_only)) {
		...
		if (lol_likely(log->print.linefeed))
...
LOL_PUBLIC void lol_printf(
	...
	lol_vprintf(level, log, domain_id, err, file, line, func, content_only, format, args);
	va_end(args);
}

static pthread_mutex_t lol_mutex = PTHREAD_MUTEX_INITIALIZER;

static lol_inline void
init_lol(lol_t *log, lol_level_e std_level, const char *file, lol_level_e file_level) {
	/* add default writer */
...
LOL_PUBLIC int lol_add_domain(
	...
	if (lol_unlikely(!lol_list || !domain || !strlen(domain)))
		return -1;
